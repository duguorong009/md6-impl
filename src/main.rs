...
fn hash(size: usize, data: &[u8], key: &[u8], levels: usize) -> Vec<u8> {
    ...
    let S0 = 0x0123456789abcdef;
    let Sm = 0x7311c2812425cfa0;
    ...
    let t  = [17, 18, 21, 31, 67, 89];
    let rs = [10,  5, 13, 10, 11, 12,  2,  7, 14, 15,  7, 13, 11,  7,  6, 12];
    let ls = [11, 24,  9, 16, 15,  9, 27, 15,  6,  2, 29,  8, 15,  5, 31,  9];
    ...
    let mid = |B: Vec<u64>, C: Vec<u64>, i: u64, p: u64, z: u64| {
    ...
    todo!("SHOULD REFACTOR since closure borrowing causes compiling error")
}
