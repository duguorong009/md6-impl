...
impl MD6State {
    ...
    pub fn update(&mut self, data: &[u8], databitlen: usize) {
        ...
        while j < databitlen {
            ...
            let mut block_words: [md6_word; b] = [0x00; b];
            if portion_size == b * w {
    ...
    fn standard_compress(
        ...
        let mut N: [md6_word; md6_n] = [0; md6_n];
        let mut A: [md6_word; 5000] = [0; 5000];
...
fn test_md6() {
    ...
    for (text, size, expected_hash) in TEST_VECTORS {
        ...
        let hex_output = output[0..size / 8]
            .into_iter()
            .map(|o| format(format_args!("{:02x}", o)))
            .collect::<String>();
